    ...
    <>
      ...
      <article className="pb-4 pt-12">
        <nav className="font-ui text-sm font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">
          ...
          <Link to={categoryHref} className="text-slate-950 dark:text-white">{category}</Link>
        ...
        <div className="mt-10 flex flex-wrap items-end justify-between gap-6">
          <div className="flex items-center gap-4">
            {author?.photo_url ? (
              ...
              <div className="grid h-14 w-14 place-items-center rounded-full bg-slate-200 font-ui text-sm font-bold text-slate-500 dark:bg-slate-700 dark:text-slate-300">
                FXLFM
              </div>
            )}
            <div>
              <p className="font-ui text-xs text-slate-500 dark:text-slate-400">Author</p>
              <p className="font-ui text-lg font-bold">{author?.name || article.source_name || "FXLFM"}</p>
            </div>
          </div>
        ...
        <div className="mt-24 grid gap-16 lg:grid-cols-[1fr_360px]">
          <div>
            ...
            <div className="mt-12 space-y-8 font-ui text-2xl leading-relaxed text-slate-900 dark:text-slate-200">
              {(paragraphs.length ? paragraphs : [article.summary]).filter(Boolean).map((paragraph, idx) => (
